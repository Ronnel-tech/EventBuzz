...
<body class="public-page bg-[#151419]">
    ...
    <nav class="page-nav">
        ...
        <div class="page-nav-links justify-end">
            <a href="<?= url('/attendee/detailed_event?id=' . $event['id']) ?>"
                class="text-secondary transition hover:text-white">Back to Event</a>
    ...
    <div class="flex min-h-[calc(100vh-8rem)] items-center justify-center">
        <div class="grid w-full max-w-7xl grid-cols-1 gap-6 lg:grid-cols-3">
            <form id="checkoutForm" method="POST" action="<?= url('/attendee/checkout?id=' . $event['id']) ?>"
                class="rounded-2xl border border-gray-800 bg-surface p-6 lg:col-span-2">
                <?= csrf_field() ?>

                <div class="mb-6 border-b border-[#2a2a2e] pb-6 text-center">
                    <h2 class="text-2xl font-semibold"><?= esc($event['title']) ?></h2>
                    <p class="text-sm text-secondary"><?= esc(date('l, F d, Y', strtotime($event['start_datetime']))) ?>
                    </p>
                ...
                <div class="space-y-4">
                    <?php if ($ticket_types): ?>
                        <?php foreach ($ticket_types as $ticket): ?>
                            ...
                            ?>
                            <div
                                class="flex flex-col justify-between gap-4 rounded-2xl border border-[#2a2a2e] bg-[#151419] p-5 md:flex-row md:items-center">
                                <div>
                                    <h3 class="text-lg"><?= esc($ticket['name']) ?></h3>
                                    <p class="text-sm text-gray-300">
                                        <?= esc(format_checkout_price($ticket['price'], $is_free_event)) ?></p>
                                    <p class="text-xs text-gray-500">Sales start:
                                        <?= esc(date('M d, Y h:i A', strtotime($ticket['start_datetime']))) ?></p>
                                    <p class="text-xs text-gray-500">Sales end:
                                        <?= esc(date('M d, Y h:i A', strtotime($ticket['end_datetime']))) ?></p>
                                </div>
                                ...
                                <div class="text-left md:text-right">
                                    ...
                                    <div class="mt-4 flex items-center gap-3 md:justify-end">
                                        <label for="quantity_<?= esc((string) $ticket['id']) ?>"
                                            class="text-sm text-gray-300">Quantity</label>
                                        <input id="quantity_<?= esc((string) $ticket['id']) ?>" type="number"
                                            name="quantities[<?= esc((string) $ticket['id']) ?>]"
                                            value="<?= esc((string) ((int) ($selected_quantities[$ticket['id']] ?? 0))) ?>"
                                            min="0" max="<?= esc((string) $ticket_left) ?>" data-ticket-quantity
                    ...
                    <?php else: ?>
                        <div
                            class="rounded-2xl border border-dashed border-[#2a2a2e] bg-[#151419] p-6 text-center text-gray-400">
                ...
                <div class="flex flex-1 flex-col justify-between p-6">
                    <div>
                        ...
                        <div class="flex justify-between text-lg font-semibold">
                            <span>TOTAL</span>
                            <span
                                id="checkoutTotal"><?= esc(format_checkout_price($starting_total, $is_free_event)) ?></span>
                        </div>
                    </div>

                    <?php if ($ticket_types): ?>
                        <button type="submit" form="checkoutForm"
                            class="mt-6 rounded-full bg-yellow-400 py-3 font-semibold text-black transition hover:bg-yellow-300">
                            Continue to Payment
                        </button>
                    <?php else: ?>
                        <button
                            class="mt-6 cursor-not-allowed rounded-full bg-[#2a2a2e] py-3 font-semibold text-secondary opacity-70"
                            disabled>

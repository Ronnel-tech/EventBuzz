...
<div class="min-h-screen flex items-center justify-center ">
  <div class="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <!-- LEFT SIDE -->
    <form id="checkoutForm" method="POST" action="<?= url('/attendee/checkout?id=' . $event['id']) ?>" class="lg:col-span-2 bg-surface rounded-2xl p-6 border border-gray-800">
      <?= csrf_field() ?>

      <!-- Header -->
      <div class="mb-6 border-b border-[#2a2a2e] pb-6 text-center">
        <h2 class="text-2xl font-semibold"><?= esc($event['title']) ?></h2>
        <p class="text-secondary text-sm"><?= esc(date('l, F d, Y', strtotime($event['start_datetime']))) ?></p>
        ...
        <?php if ($ticket_types): ?>
            ...
            <div class="flex flex-col justify-between gap-4 rounded-2xl border border-[#2a2a2e] bg-[#151419] p-5 md:flex-row md:items-center">
                ...
                <div class="text-left md:text-right">
                    <p class="text-sm text-gray-400">Tickets Left: <?= esc((string) $ticket['tickets_left']) ?></p>
                    <p class="text-sm text-gray-400">Total Quantity: <?= esc((string) $ticket['quantity']) ?></p>
                    <p class="text-sm text-gray-400">Sold: <?= esc((string) $ticket['sold']) ?></p>
    ...
    <!-- RIGHT SIDE -->
    <div class="bg-[#151821] rounded-2xl border border-gray-800 overflow-hidden flex flex-col">

      <!-- Image -->
      <img src="<?= esc($banner_image) ?>"
           class="h-40 w-full object-cover" alt="<?= esc($event['title']) ?>">
      ...
      <div class="p-6 flex flex-col flex-1 justify-between">
        ...
        <div>
          ...
          <div class="space-y-2 text-sm text-gray-300">
            ...
            <div class="flex justify-between">
              <span>Payment</span>
              <span><?= esc(ucfirst($payment_type ?: 'Unavailable')) ?></span>
            </div>
          </div>
        ...
        <!-- Button -->
        <?php if ($ticket_types): ?>
            <button type="submit" form="checkoutForm" class="mt-6 bg-yellow-400 text-black py-3 rounded-full font-semibold hover:bg-yellow-300 transition">
              Buy Tickets
            </button>
        <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderSummary() {
        ...
        if (!selectedLines.length) {
            summaryContainer.innerHTML = '<div class="flex justify-between text-gray-500"><span>No tickets selected yet</span><span>PHP 0.00</span></div>';

...
<nav class="page-nav">
    ...
    <div class="page-nav-links justify-end">
        <a href="<?= url('/attendee/checkout?id=' . $event['id']) ?>" class="text-secondary transition hover:text-white">Back to Checkout</a>
...
<div class="mx-auto grid max-w-7xl gap-6 lg:grid-cols-3">
    <form method="POST" action="<?= url('/attendee/payment?id=' . $event['id']) ?>" enctype="multipart/form-data" class="rounded-3xl border border-[#2a2a2e] bg-surface p-6 lg:col-span-2">
        <?= csrf_field() ?>
        ...
        <div class="grid gap-4 md:grid-cols-<?= count($payment_method_options) > 1 ? '2' : '1' ?>">
            ...
            <label class="relative cursor-pointer rounded-2xl border border-white/10 bg-[#151419] p-5 transition hover:border-white/20">
                <input
                    type="radio"
                    name="payment_method"
                    value="<?= esc($payment_method_option) ?>"
                    class="peer sr-only"
                    <?= $old['payment_method'] === $payment_method_option ? 'checked' : '' ?>
                >
        ...
        <div id="gcashFields" class="mt-6 rounded-3xl border border-white/10 bg-[#12121a] p-5 md:p-6 <?= $old['payment_method'] === 'gcash' ? '' : 'hidden' ?>">
            <div class="mb-5">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <input
                    ...
                    value="<?= esc($old['gcash_sender_name']) ?>"
            ...
            <div class="mt-6 grid gap-6 lg:grid-cols-2">
                ...
                <div class="rounded-2xl border border-white/10 bg-[#0d0d14] p-5">
                    ...
                    <p class="text-sm text-white/60">Name: <span class="text-white"><?= esc($event['gcash_name'] ?: 'Not provided') ?></span></p>
                    <p class="pt-2 text-sm text-white/60">Number: <span class="text-white"><?= esc($event['gcash_number'] ?: 'Not provided') ?></span></p>
                    <?php if (!empty($event['gcash_qr'])): ?>
                    <img src="<?= esc($event['gcash_qr']) ?>" alt="Organizer GCash QR" class="mt-4 h-56 w-full rounded-2xl object-cover outline outline-[#2a2a2e] shadow-soft">
                    <?php else: ?>
        ...
        <div class="mt-8 flex justify-end">
            <button type="submit" class="rounded-full bg-yellow-400 px-8 py-3 text-sm font-semibold text-black transition hover:brightness-110">
        ...
        <div class="p-6">
            ...
            <div class="space-y-2 text-sm text-gray-300">
                ...
                <div class="flex justify-between gap-3">
                    <span><?= esc((string) $line_item['quantity']) ?>x <?= esc($line_item['ticket']['name']) ?></span>

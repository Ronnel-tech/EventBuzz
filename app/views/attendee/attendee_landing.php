...
<body class="public-page-wide">
    ...
    <nav class="page-nav bg-surface-hover">
        ...
        <div class="page-nav-links justify-end">
            ...
            <form method="GET" action="<?= url('/attendee') ?>" class="flex items-center gap-2">
                <input type="text" name="search" value="<?= esc($search_value) ?>" placeholder="Search events..."
    ...
    <div class="relative mb-1 min-h-[70vh] overflow-hidden rounded-4xl pt-16 sm:pt-20 lg:pt-30">
        ...
        <!-- Blurred background -->
        <div class="absolute inset-0 bg-cover bg-center blur-3xl"
            style="background-image: url('<?= esc($hero_image) ?>');"></div>
        <div class="absolute inset-0 bg-black/45"></div>

        <!-- Content (NOT blurred) -->
        <div
            class="relative z-10 flex h-full flex-col items-center justify-center p-5 pt-16 text-center sm:pt-24 lg:pt-30">
                ...
                <div class="grid gap-6 md:grid-cols-3 xl:grid-cols-5">
                    <?php foreach ($today_events as $event): ?>
                        <a href="<?= url('/attendee/detailed_event?id=' . $event['id']) ?>"
                            class="overflow-hidden rounded-3xl bg-[#1c2029] shadow-soft outline outline-[#2a2a2e] transition hover:-translate-y-1">
                            <div class="h-52 w-full bg-[#151419] bg-cover bg-center"
                                style="background-image: url('<?= esc((string) ($event['banner_image'] ?: '/public/assets/images/logo.png')) ?>');">
                            </div>
                            <div class="space-y-3 p-5">
                                <div class="flex items-center justify-between gap-3">
                                    <span
                                        class="rounded-full bg-white/10 px-3 py-1 text-xs text-gray-300"><?= esc(date('M d, Y g:i A', strtotime($event['start_datetime']))) ?></span>
                                ...
                                <p class="text-lg font-semibold text-white">
                                    <?= esc(format_attendee_price($event['starting_price'], (string) $event['payment_type'])) ?>
                                </p>
                            </div>
                        </a>
                    <?php endforeach; ?>

                ...
                <?php if ($events): ?>
                    <div class="divide-y divide-[#2a2a2e] bg-surface">
                        <?php foreach ($events as $event): ?>
                            <div class="grid cursor-pointer items-center gap-4 px-6 py-5 text-sm text-white transition hover:bg-[#151419]"
                                style="grid-template-columns: minmax(220px, 1.3fr) minmax(180px, 1fr) minmax(140px, 0.9fr) minmax(180px, 1fr) minmax(120px, 0.7fr) minmax(120px, 0.7fr);"
                                ...
                                <div class="text-right">
                                    <form method="POST" action="<?= url('/admin/event-management') ?>" class="inline-block"
                                        onclick="event.stopPropagation();"
                                        onsubmit="return confirm('Delete this event and all related ticket sales, orders, and tickets?');">
                                        <?= csrf_field() ?>

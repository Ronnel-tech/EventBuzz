        ...
        <section class="rounded-3xl bg-surface p-8 outline outline-[#2a2a2e] shadow-soft">
            ...
            <div class="table-scroll rounded-2xl border border-[#2a2a2e]">
                <div class="grid gap-4 border-b border-[#2a2a2e] bg-[#151419] px-6 py-4 text-sm text-secondary"
                    style="grid-template-columns: minmax(200px, 1.2fr) minmax(220px, 1.2fr) minmax(140px, 0.8fr) minmax(120px, 0.7fr) minmax(120px, 0.7fr);">
                    <div>Organizer</div>
                    <div>Email</div>
                    <div>Joined On</div>
                        ...
                        <?php foreach ($organizers as $organizer): ?>
                            ...
                            <div class="grid items-center gap-4 px-6 py-5 text-sm text-white"
                                ...
                                <div class="text-right">
                                    <form method="POST" action="<?= url('/admin/organizer-management') ?>" class="inline-block"
                                        onsubmit="return confirm('Delete this organizer and all related events, ticket types, and purchases?');">
                                        <?= csrf_field() ?>
                                        <input type="hidden" name="action" value="delete">
                                        <input type="hidden" name="organizer_id" value="<?= esc((string) $organizer['id']) ?>">
                                        <button type="submit"
                                            class="rounded-full border border-red-400/40 bg-red-500/10 px-4 py-2 text-xs font-semibold text-red-300 transition hover:bg-red-500/20">
                                            Delete
                                        </button>
                                    </form>
                                </div>

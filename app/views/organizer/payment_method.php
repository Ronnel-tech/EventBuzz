...
<head>
    ...
    <title>EventBuzz | Payment Method</title>
...
<body class="app-shell">
    ...
    <aside class="app-sidebar">
        ...
        <div class="app-sidebar-nav">
            ...
            <button><svg id="payment_method" class="icon-primary" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28"><path d="M5.754 5a3.75 3.75 0 0 0-3.75 3.75v.75H26v-.75A3.75 3.75 0 0 0 22.25 5zm-3.75 14.25V11H26v8.25A3.75 3.75 0 0 1 22.25 23H5.755a3.75 3.75 0 0 1-3.75-3.75M18.25 16.5a.75.75 0 0 0 0 1.5h3.5a.75.75 0 0 0 0-1.5z"/></svg></button>
        </div>
    ...
    <div class="app-main">
        ...
        <section class="app-section">
            ...
            <section class="w-full modal outline outline-[#2a2a2e] shadow-soft ">
                ...
                <form action="<?= url('/organizer/payment-method') ?>" method="POST" enctype="multipart/form-data" class="p-8">
                    <?php csrf_field() ?>
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                        <label class="relative cursor-pointer rounded-2xl border border-white/10 bg-[#0d0d14] p-5 transition hover:border-white/20">
                            <input type="radio" name="payment_type" value="free" class="peer sr-only" <?= $old['payment_type'] === 'free' ? 'checked' : '' ?> />
                        ...
                        <label class="relative cursor-pointer rounded-2xl border border-white/10 bg-[#0d0d14] p-5 transition hover:border-white/20">
                            <input type="radio" name="payment_type" value="cash" class="peer sr-only" <?= $old['payment_type'] === 'cash' ? 'checked' : '' ?> />
                            <div class="absolute right-4 top-4 h-4 w-4 rounded-md border border-white/40 bg-transparent peer-checked:border-yellow-400 peer-checked:bg-yellow-400"></div>
                            <div>
                                <h3 class="text-base font-medium">Cash</h3>
                                <p class="mt-1 text-sm text-white/45">Pay at the venue</p>
                            </div>
                        </label>

                        <label class="relative cursor-pointer rounded-2xl border border-white/10 bg-[#0d0d14] p-5 transition hover:border-white/20">
                            <input type="radio" name="payment_type" value="gcash" class="peer sr-only" <?= $old['payment_type'] === 'gcash' ? 'checked' : '' ?> />
                            <div class="absolute right-4 top-4 h-4 w-4 rounded-md border border-white/40 bg-transparent peer-checked:border-yellow-400 peer-checked:bg-yellow-400"></div>
                            <div>
                                <h3 class="text-base font-medium">GCash</h3>
                                <p class="mt-1 text-sm text-white/45">Pay through online transfer</p>
                            </div>
                        </label>
                    </div>
                    ...
                    <div class="mt-8 rounded-3xl border border-white/10 bg-[#12121a] p-5 md:p-6">
                        ...
                        <div class="mt-6">
                            ...
                            <div class="w-full h-50 inset-0 bg-[url('/public/assets/images/signup_bg.jpg')] bg-cover bg-center rounded-t-3xl outline outline-[#2a2a2e] shadow-soft mb-1 flex items-center justify-center shadow-soft">
                            ...
                            <?php if (!empty($profile['gcash_qr'])): ?>
                            <div class="mt-4">
                                <p class="mb-3 text-sm text-white/60">Current uploaded QR</p>
                                <img src="<?= esc($profile['gcash_qr']) ?>" alt="Saved GCash QR" class="h-40 rounded-2xl object-cover outline outline-[#2a2a2e] shadow-soft">
                            </div>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="mt-10 flex justify-end">
                        <button type="submit" class="min-w-47.5 rounded-full bg-yellow-400 px-8 py-3 text-sm font-semibold text-black transition hover:brightness-110">
                            Publish
                        </button>
                    </div>
                </form>

                    ...
                    <div class="mb-8 border-b border-[#2a2a2e] pb-6">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            <div>
                                ...
                                <p class="pt-2 text-sm text-secondary">Order
                                    #<?= esc((string) $payment_details['id']) ?></p>
                            </div>

                            <form method="POST"
                                action="<?= url('/organizer/qr_page?event_id=' . $event['id'] . '&order_id=' . $payment_details['id']) ?>"
                    ...
                    <div class="grid gap-5 md:grid-cols-2">
                        <div>
                            ...
                            <p class="pt-1 text-white">
                                <?= esc($payment_details['attendee_name'] ?: 'Unknown attendee') ?></p>
                        </div>
                        <div>
                            <p class="text-sm text-secondary">Transaction Date</p>
                            <p class="pt-1 text-white">
                                <?= esc(date('F d, Y h:i A', strtotime($payment_details['transaction_date']))) ?></p>
                        </div>
                        <div>
                            <p class="text-sm text-secondary">Payment Method</p>
                            <p class="pt-1 text-white"><?= esc(ucfirst((string) $payment_details['payment_method'])) ?>
                            </p>
                        </div>
                        <div>
                            <p class="text-sm text-secondary">Total Amount</p>
                            <p class="pt-1 text-white">PHP
                                <?= esc(number_format((float) $payment_details['total_amount'], 2)) ?></p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="pt-1 text-white">
                                <?= esc($payment_details['gcash_reference'] !== '' ? $payment_details['gcash_reference'] : 'N/A') ?>
                            </p>
                        </div>
                    </div>
                ...
                <div class="rounded-2xl bg-surface p-8 outline outline-[#2a2a2e] shadow-soft">
                    ...
                    <?php if ((string) $payment_details['payment_method'] === 'gcash' && !empty($payment_details['gcash_screenshot'])): ?>
                        <div class="mt-6 overflow-hidden rounded-2xl bg-[#151419]">
                            <img src="<?= esc($payment_details['gcash_screenshot']) ?>" alt="GCash receipt"
                                class="w-full object-cover">
                        </div>
                    <?php else: ?>
                        <div
                            class="mt-6 rounded-2xl border border-dashed border-[#2a2a2e] bg-[#151419] p-8 text-center text-secondary">

        ...
        <main class="app-main">
            <div class="mb-8 flex items-end justify-between gap-6">
                <div>
                    ...
                    <p class="pt-3 text-secondary">Performance summary for <?= esc($display_name) ?> and all
                        organizer-owned events.</p>
                </div>
            ...
            <section class="mt-8 grid gap-6 xl:grid-cols-2">
                <article class="rounded-3xl border border-[#2a2a2e] bg-surface p-6 shadow-soft">
                    <div class="mb-5">
                        ...
                        <p class="pt-2 text-sm text-secondary">Line graph of today&apos;s completed ticket sales for
                            each event.</p>
                    </div>
                    <div class="rounded-2xl bg-[#111117] p-4">
                        <svg id="ticketsTodayChart" viewBox="0 0 760 320" class="h-80 w-full" role="img"
                            aria-label="Sold tickets today per event chart"></svg>
                    </div>
                </article>
                ...
                <article class="rounded-3xl border border-[#2a2a2e] bg-surface p-6 shadow-soft">
                    <div class="mb-5">
                        ...
                        <p class="pt-2 text-sm text-secondary">Line graph of completed order revenue by transaction
                            date.</p>
                    </div>
                    <div class="rounded-2xl bg-[#111117] p-4">
                        <svg id="revenueChart" viewBox="0 0 760 320" class="h-80 w-full" role="img"
                            aria-label="Revenue over time chart"></svg>
                    </div>
                </article>
            </section>
            ...
            <section class="mt-8 grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
                <article class="rounded-3xl border border-[#2a2a2e] bg-surface p-6 shadow-soft">
                    ...
                    <div class="grid items-center gap-6 lg:grid-cols-[320px_1fr]">
                        <div class="rounded-2xl bg-[#111117] p-4">
                            <svg id="salesDistributionChart" viewBox="0 0 320 320" class="h-80 w-full" role="img"
                                aria-label="Sales distribution pie chart"></svg>
                        </div>
                ...
                <article class="rounded-3xl border border-[#2a2a2e] bg-surface p-6 shadow-soft">
                    <div class="mb-5">
                        <h3>Snapshot</h3>
                        <p class="pt-2 text-sm text-secondary">Quick reading of the current organizer activity levels.
                        </p>
                    </div>
                    <div class="space-y-4">
                        <div class="rounded-2xl bg-[#111117] p-5">
                            <p class="text-sm text-secondary">Total created events</p>
                            <p class="pt-2 text-2xl text-white">
                                <?= esc((string) ((int) ($dashboard_summary['total_events_created'] ?? 0))) ?></p>
                        </div>
                        <div class="rounded-2xl bg-[#111117] p-5">
                            <p class="text-sm text-secondary">Revenue earned from completed payments</p>
                            <p class="pt-2 text-2xl text-white">PHP
                                <?= esc(number_format((float) ($dashboard_summary['total_revenue_earned'] ?? 0), 2)) ?>
                            </p>
                        </div>
                        <div class="rounded-2xl bg-[#111117] p-5">
                            <p class="text-sm text-secondary">Upcoming events scheduled</p>
                            <p class="pt-2 text-2xl text-white">
                                <?= esc((string) ((int) ($dashboard_summary['upcoming_events'] ?? 0))) ?></p>
                        </div>
                    </div>
                </article>
            </section>

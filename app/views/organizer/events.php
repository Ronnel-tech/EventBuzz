...
<body class="app-shell">
    ...
    <div class="app-main">
        ...
        <div class="flex flex-col pr-15">
            ...
            <form method="GET" action="<?= url('/organizer/events') ?>"
                class="flex items-center justify-end align-center gap-3">
                ...
                <!-- Filter -->
                <select name="filter" class="bg-surface text-secondary px-3 py-2 rounded-full "
            ...
            <section class="app-section">
                ...
                <div
                    class="bg-surface w-full outline  outline-[#2a2a2e] shadow-soft mt-5 rounded-2xl overflow-hidden pb-10 sm:pb-16 lg:pb-30">
                    <h3 class="px-10 pt-10">Events Table</h3>

                    <div class="table-scroll px-4 py-6 sm:px-8 sm:py-8 lg:px-10">
                        <div
                            class="grid grid-cols-[2fr_1fr_1fr_80px] gap-4 border-b border-[#2a2a2e] pb-4 text-sm text-gray-400">
                            <div>Event Name</div>
                            <div>Date</div>
                                    ...
                                    <div class="grid cursor-pointer grid-cols-[2fr_1fr_1fr_80px] gap-4 py-5 text-white items-center hover:bg-[#151419]"
                                        ...
                                        <div class="flex justify-end" onclick="event.stopPropagation()">
                                            <details class="relative" onclick="event.stopPropagation()">
                                                <summary
                                                    class="flex cursor-pointer list-none items-center justify-center rounded-full p-2 hover:bg-[#2a2a2e]">
                                                ...
                                                <div
                                                    class="absolute right-0 top-10 z-10 w-36 rounded-2xl border border-[#2a2a2e] bg-surface p-2 shadow-soft">
                                                    <a href="<?= url('/organizer/detailed-event?id=' . $event['id']) ?>"
                                                        class="block rounded-xl px-4 py-2 text-sm text-white hover:bg-[#2a2a2e]">View</a>
                                                    <a href="<?= url('/organizer/edit-event?id=' . $event['id']) ?>"
                                                        class="block rounded-xl px-4 py-2 text-sm text-white hover:bg-[#2a2a2e]">Edit</a>
                                                    <form method="POST" action="<?= url('/organizer/events') ?>">
                                                        <?php csrf_field(); ?>
                                                        <input type="hidden" name="action" value="delete">
                                                        <input type="hidden" name="event_id"
                                                            value="<?= esc((string) $event['id']) ?>">
                                                        <button type="submit"
                                                            class="block w-full rounded-xl px-4 py-2 text-left text-sm text-red-300 hover:bg-[#2a2a2e]">Delete</button>
                                                    </form>
                                                </div>
                                            </details>
                                        </div>
